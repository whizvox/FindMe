...
public class FMConfig {
  ...
  public boolean shouldCheckCaches() {
    return checkCaches;
  }
  ...
  public void load(FileConfiguration config) {
    ...
    config.addDefault(CFG_CHECK_CACHES, true);
    ...
    config.setComments(CFG_CHECK_CACHES, List.of(
        "Whether to automatically validate and fix caches when fetching from them",
        "Can potentially fix issues when server is running for a long time",
        "Recommended to disable if there are many players attempting to find many items"
    ));
    ...
    checkCaches = config.getBoolean(CFG_CHECK_CACHES);
  }
  ...
  public void save(FileConfiguration config) {
    ...
    config.set(CFG_CHECK_CACHES, checkCaches);
  }
